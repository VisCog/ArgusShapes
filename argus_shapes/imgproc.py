...
def rd_loss(img_true, img_pred, max_rot=175, n_angles=71, return_all=False):
    if not isinstance(img_true, np.ndarray):
        raise TypeError("`img_true` must be a  NumPy array")
    if not isinstance(img_pred, np.ndarray):
        raise TypeError("`img_pred` must be a NumPy array")
    ...
    # Rotate the phosphene so that the dice coefficient is maximized:
    angles = np.linspace(-max_rot, max_rot, n_angles)
    dice = [dice_coeff(img_true, rotate_phosphene(img_pred, r))
            for r in angles]

    # If multiple angles give the same coefficient, choose the smallest angle.
    img_angles = angles[np.isclose(dice, np.max(dice))]
    idx_angles = np.argmin(np.abs(img_angles))
    img_angle = img_angles[idx_angles]
    img_dice = np.max(dice)

    # However, if rotation didn't make a difference, choose largest angle:
    if np.isclose(np.var(dice), 0):
        img_angle = max_rot

    # Dice loss in [0, 1], angle loss in [0, 1], so RD loss in [0, 2]:
    rd_loss = (1 - img_dice) + np.abs(img_angle) / max_rot

    if return_all:
        return rd_loss, img_angle, img_dice
    else:
        return rd_loss
