...
def test_rd_loss():
    for img0 in [1, (3, 4), [1, 2, 3], [[1, 2]], pd.Series([1, 2])]:
        with pytest.raises(TypeError):
            imgproc.rd_loss(img0, np.zeros((3, 3)))
    for img1 in [1, (3, 4), [1, 2, 3], [[1, 2]], pd.Series([1, 2])]:
        with pytest.raises(TypeError):
            imgproc.rd_loss(np.zeros((3, 3)), img1)
    for img0shape in [(3, 4), (4, 4), (5, 5)]:
        with pytest.raises(TypeError):
            imgproc.rd_loss(np.zeros(img0shape), np.zeros((3, 3)))
    for img1shape in [(3, 4), (4, 4), (5, 5)]:
        with pytest.raises(TypeError):
            imgproc.rd_loss(np.zeros((3, 3)), np.zeros(img1shape))

    # No overlap gives max error:
    for img0, img1 in zip([np.zeros((7, 8)), np.ones((7, 8))],
                          [np.ones((7, 8)), np.zeros((7, 8))]):
        loss, angle, dice = imgproc.rd_loss(img0, img1, return_all=True)
        npt.assert_almost_equal(np.abs(angle), 175)
        npt.assert_almost_equal(dice, 0)
        npt.assert_almost_equal(loss, 2)

    # Identical images gives zero error:
    img = np.zeros((17, 28))
    img[5:9, 14:19] = 1
    loss, angle, dice = imgproc.rd_loss(img, img, return_all=True)
    npt.assert_almost_equal(np.abs(angle), 0)
    npt.assert_almost_equal(dice, 1)
    npt.assert_almost_equal(loss, 0)

    # A real, simple example:
    img0 = np.zeros((31, 49))
    rr, cc = skid.ellipse(img0.shape[0] // 2, img0.shape[1] // 2, 9, 5,
                          shape=img0.shape)
    img0[rr, cc] = 1
    rot_angle = -15
    img1 = imgproc.rotate_phosphene(img0, rot_angle)
    loss, angle, dice = imgproc.rd_loss(img0, img1, return_all=True)
    npt.assert_almost_equal(angle, -rot_angle)
    npt.assert_almost_equal(dice, 1)
    npt.assert_almost_equal(loss, np.abs(rot_angle) / 175)
