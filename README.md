## p2pspatial
[![Build Status](https://travis-ci.org/uwescience/p2pspatial.svg?branch=master)](https://travis-ci.org/uwescience/p2pspatial)
...
instructions at the [bottom of this page](#using-p2pspatial-as-a-template).
...
`p2pspatial/p2pspatial.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `p2pspatial.py` in directory called
`p2pspatial`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import p2pspatial as sb` in an
interactive Python session, the classes and functions defined inside of the
`p2pspatial.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the p2pspatial module, the main classes defined at the bottom of
...
Either way, you can create a `p2pspatial/data` folder in which you can
...
(typically, something like our `p2pspatial/tests/test_p2pspatial.py`). Within each
...
have one such test in `p2pspatial/tests/test_p2pspatial.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/p2pspatial`),
and type:
...
will see a message such as:
    ...
    p2pspatial/tests/test_p2pspatial.py .F...
    ...
    p2pspatial/tests/test_p2pspatial.py:49: AssertionError
...
To document `p2pspatial` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/p2pspatial/blob/master/doc/_static/RTD-advanced-conf.png)

 http://p2pspatial.readthedocs.org/en/latest/
...
A `p2pspatial/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/p2pspatial) for the software. This
...
`p2pspatial` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `p2pspatial`, we use the
...
[here](https://github.com/uwescience/p2pspatial/blob/master/scripts/Figure1.ipynb).
...
### Using `p2pspatial` as a template
...
Under "old repository clone URL" enter: https://github.com/uwescience/p2pspatial
...
`p2pspatial/p2pspatial.py` to be called `smallish/smallish.py`

	git mv p2pspatial smallish
	git mv smallish/p2pspatial.py smallish/smallish.py
	git mv smallish/tests/test_p2pspatial.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `p2pspatial` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `p2pspatial` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `p2pspatial` in them. Type
the following to see where all these files are:

	git grep p2pspatial

You can replace `p2pspatial` for `smallish` quickly with:

	git grep -l 'p2pspatial' | xargs sed -i 's/p2pspatial/smallish/g'
...
Edit all the mentions of `p2pspatial` in the `p2pspatial/__init__.py` file, and in
the `p2pspatial/version.py` file as well.
